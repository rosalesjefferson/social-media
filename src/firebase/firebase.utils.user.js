...
export const updateProfileInfo = async payload =>{
  ...
  await usersCollectionRef.get().then(snapShot =>{
    ...
    // GET EACH USER'S ID TO GET EACH USER'S "FOLLOWING" DATABASE
      ...
      // UPDATE FOLLOWERS NAME
      usersCollectionRef.doc(userDoc.id).collection('followers').get().then(followerSnapshot =>{
        followerSnapshot.forEach(followDoc =>{
          if(email === followDoc.data().email){
            usersCollectionRef.doc(userDoc.id).collection('followers').doc(followDoc.id).update({
              firstName: uFirstName,
              lastName: uLastName,
              userDP: profileUrl
            })
